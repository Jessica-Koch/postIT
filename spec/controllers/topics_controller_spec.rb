...
RSpec.describe TopicsController, type: :controller do
    ...
    context "guest" do
        ...
        describe "POST create" do
            it "returns http redirect" do
                post :create, params: {topic: {name: Faker::Hipster.sentence(2), description: Faker::Hipster.paragraph(3)}}
                expect(response).to redirect_to(new_session_path)
            end
        end
    ...
    context "member user" do
        before do
            user = User.create!(name: Faker::Name.name, email: Faker::Internet.email, password: Faker::Internet.password, role: :member)
            create_session(user)
        end
        ...
        describe "PUT update" do
            it "returns http redirect" do
                ...
                put :update, params: {id: my_topic.id, topic:{
                    name: new_name,
    ...
    context "moderator users" do
        before do
            user = User.create!(name: "PostIt User", email: "user@postit.com", password: "password", role: :moderator)
            create_session(user)
        end
    ...
    context "admin users" do
        before do
            user = User.create!(name: "PostIt User", email: "user@postit.com", password: "password", role: :admin)
            create_session(user)
        end
        ...
        describe "POST create" do
            it "increases the number of topics by 1" do
                expect{ post :create, params: {topic: {name: Faker::Hipster.sentence(2), description: Faker::Hipster.paragraph(3)}} }.to change(Topic,:count).by(1)
            end
